...
const PendingTasksScreen = ({ isDarkMode, navigation }) => {
  ...
  const animatedValues = pendingTasks.reduce((acc, task) => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <Text style={[styles.title, { color: theme.text }]}>Pending Tasks</Text>
      <View style={styles.filterContainer}>
        ...
        <Picker
          ...
          dropdownIconColor={theme.text}
        >
          <Picker.Item label="Sort by Creation" value="createdAt" />
      ...
      {filteredTasks.length === 0 ? (
        <Text style={[styles.noTasksText, { color: theme.text }]}>No pending tasks 🎉</Text>
      ) : (
        <FlatList
          data={filteredTasks}
